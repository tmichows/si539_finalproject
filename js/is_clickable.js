window.addEventListener("load", function() {
    ...
    const indirect_statement_structural_vocab = ["putat", "videtis", "videatis",
"spero", "scio"]
    var number_of_indirect_statement_words = indirect_statement_structural_vocab.length

    //Defining Page Specific Other Structural Vocabulary
    const other_structural_vocab = ["cupere", "debemus", "patiatur",
     "posse","tot", "petimus", "debet", "videatur"]

    //Setup the counter
    document.getElementById("score").textContent = "There are " + 
    number_of_indirect_statement_words + " structural vocabulary words left.";
    ...
    function wordClicker(event){
        ...
        //Correct Word
        if (indirect_statement_structural_vocab.includes(clickedWord)) {
            event.target.classList.add('correct_sv');
            //Update Counter
            number_of_indirect_statement_words--;
            document.getElementById("score").textContent = 
            "There are " + number_of_indirect_statement_words
        ...
        if(number_of_indirect_statement_words == 0)
        {
