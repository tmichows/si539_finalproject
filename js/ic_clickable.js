window.addEventListener("load", function() {
    ...
    const indirect_command_structural_vocab = ["quaeso","patiamini","patiatur", 
    "cupere"]
    var number_of_indirect_command_words = indirect_command_structural_vocab.length

    //Defining Page Specific Other Structural Vocabulary
    const other_structural_vocab = ["quaereretur", "putabat", "posse", "dixisse",
"videatur","uti", "putat", "debemus"]
    
    //Setup the counter
    document.getElementById("score").textContent = "There are " + 
    number_of_indirect_command_words + " structural vocabulary words left.";
    ...
    function wordClicker(event){
        ...
        //Correct Word
        if (indirect_command_structural_vocab.includes(clickedWord)) {
            event.target.classList.add('correct_sv');
             //Update Counter
             number_of_indirect_command_words--;
             document.getElementById("score").textContent = 
             "There are " + number_of_indirect_command_words
        ...
        if(number_of_indirect_command_words == 0)
        {
